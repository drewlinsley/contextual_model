...
time = np.repeat(np.arange(data['y'].shape[1])[None, :], data['y'].shape[0], axis=0)
time = time.reshape(-1, 1)

group = np.repeat(np.arange(len(data['y']))[:, None], data['y'].shape[1], axis=1)
group = group.reshape(-1, 1)

long_y = data['y'].reshape(-1, 1)

lines = plt.plot(data['y'].transpose())
plt.setp(lines, alpha=0.1, linewidth=0.5, color='black')
plt.savefig('multi_sub_f3a.pdf')

# df = pd.DataFrame(np.hstack((time, group, long_y)), columns=['time', 'group', 'y'])
